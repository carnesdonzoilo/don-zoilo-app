// DON ZOILO V35.3.33 — ESTADÍSTICAS
...
(() => {
  ...
  // V35.3.33: unificación de nombres equivalentes SOLO para el ranking de clientes.
  ...
  // V35.3.66: equivalencias revisadas para el ranking de septiembre 2026.
  // Solo afectan Estadísticas/PDF; no renombran productos en pedidos, stock ni precios.
  addProductGroup('Paleta', ['paleta trozo']);
  addProductGroup('Bife angosto', ['bife angosto 1 de dedo']);
  addProductGroup('Pata y muslo', ['pata muslo']);
  addProductGroup('Asado', ['asado banderita']);
  addProductGroup('Matambrito', ['matambre de cerdo']);
  addProductGroup('Morcilla', ['morcilla atada']);
  addProductGroup('Parrillera', ['salchicha parrillera']);
  addProductGroup('Alita', ['alitas']);
  ...
  function fillClients(){
    ...
    if(!select) return;
    const current=canonicalClient(select.value);
    // El selector usa los mismos nombres unificados que el Ranking de clientes.
    const clients=[...new Set(deliveredRows().map(x=>canonicalClient(x.client)).filter(Boolean))]
  ...
  // V35.3.33: una única fuente de cálculo para Resumen y rankings.
  function lineKg(o){
  ...
  function renderStatistics(){
    ...
    const rows=periodRows.filter(o=>!client || clientKey(canonicalClient(o.client))===clientKey(canonicalClient(client)));
